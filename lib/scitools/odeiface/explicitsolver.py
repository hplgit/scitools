...
class ODESolver(object):
    ...
    The information on valid parameter names, help, and type is automatically
    ...
    values to all valid parameters (i.e., those defined in self._prm).
...
'file': the y and t values are stored in a file-based database.""",
    ...
    def set(self, **kwargs):
        """Set valid solver parameters (as keyword arguments)."""
        # use techniques from help.easyviz.PrmDictBase 
		# (but make things simpler here)
        ...
        if name in type_check:
            if isinstance(type_check[name], int):
                ...
                raise TypeError, '%s._prm_type["%s"] has an '\
                      'ilvalid value %s' % (self.__class__.__name__,
                                            name, type_check[prm])
        else:
            can_set = True
    ...
    def get_safe(self, *prm_names, **kwargs):
        """
        As get, but None values indicate uninitialized/ilvalid values
