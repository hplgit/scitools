...
_registered_explicit_ode_algorithms = [funcname \
...
class ExplicitSolver(ODESolver):
    ...
    def __init__(self, **kwargs):
        self.algorithm = kwargs.get('algorithm', 'ForwardEuler') + \
                         '_algorithm'
        if self.algorithm not in _registered_explicit_ode_algorithms:
            raise ValueError, 'algorithm %s is not available '\
                  '(must be one of %s)' % \
                  (self.algorithm, str(_registered_explicit_ode_algorithms)[1:-1])
...
# make subclasses implementing all the methods in the explicit_ode_algorithms
# module:
# (use the functions' doc strings in explicit_ode_algorithms as doc strings
...
for _algorithm in _registered_explicit_ode_algorithms:
